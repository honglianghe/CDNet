...
class LabelEncoding(object):
    ...
    def __call__(self, imgs):
        ...
        if (self.out_c != 3):
            ...
            # input = instance label
            if (label_level_len > 2):
                postproc = 0
                if (postproc == 0):
                    label_instance = label_inside  # marker[:,:,0]
                    label_instance = measure.label(label_instance)
                else:
                    label_instance = postproc_other.process(label_inside.astype(np.uint8) * 255,
                                                      model_mode='modelName',
                                                      min_size=5)

                new_label = np.zeros((label.shape[0], label.shape[1]), dtype=np.uint8)
                new_label[label_instance > 0] = 1  # inside
        ...
        if (do_direction == 1):
            ...
            # distance_map = distance_transform_edt(new_label_inside)
            # distance_map: the distance of pixel-to-boundary； distance_center_map: the distance of pixel-to-center
            ...
            if (do_direction == 1):  # new_label_InsideAndEdge -> new_label_inside
                distance_map = distance_map * (new_label_inside > 0) * 255
                distance_center_map = distance_center_map * (new_label_inside > 0) * 255
                ...
                # Image_label_direction = Image.fromarray((label_direction_new).astype(np.uint8))
